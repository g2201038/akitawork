...
def load_data():
    ...
    if os.path.exists(DATA_FILE):
        ...
        try:
            ...
            st.warning(f"データ読み込みに失敗したため、初期状態で起動します: {e}")

    return {"users": {}, "jobs": {}}

def save_data(data):

    """一時ファイルに書いてから置き換えることで、書き込み途中のクラッシュや

    同時書き込みによるファイル破損を防ぐ（簡易的な原子的書き込み）。"""
...
def get_admin_credentials():
    ...
    """管理者情報はソースに直書きせず st.secrets から取得する。

    未設定の場合のみ開発用デフォルトにフォールバックする。"""
...
def show_login():
    ...
    with tab1:
        ...
        with st.container(border=True):
            ...
            if st.button("ログインする", type="primary", use_container_width=True):
                ...
                attempts = st.session_state.login_attempts.get(phone, {"count": 0, "locked_until": None})

                now = get_japan_now()

                if attempts["locked_until"] and now < attempts["locked_until"]:

                    remain = int((attempts["locked_until"] - now).total_seconds() // 60) + 1

                    st.error(f"試行回数が上限に達しました。あと約{remain}分後に再試行してください。")

                else:

                    user = db["users"].get(phone)

                    if user and user.get("pass_hash") == hash_password(password, user.get("salt", "")):

                        st.session_state.user = user

                        st.session_state.phone = phone

                        st.session_state.login_attempts[phone] = {"count": 0, "locked_until": None}
                        ...
                        if attempts["count"] >= MAX_LOGIN_ATTEMPTS:
                            ...
                            attempts["locked_until"] = now + datetime.timedelta(minutes=LOCKOUT_MINUTES)

                            st.error("試行回数の上限に達したため、一時的にロックされました。")

                        else:

                            st.error(f"電話番号または暗証番号が違います。（残り試行回数: {MAX_LOGIN_ATTEMPTS - attempts['count']}回）")

                        st.session_state.login_attempts[phone] = attempts
    ...
    with tab2:
        ...
        with st.container(border=True):
            ...
            admin_id_input = st.text_input("管理者ID", key="log_admin_id")

            admin_pass_input = st.text_input("パスワード", type="password", key="log_admin_pass")

            st.write("")

            if st.button("管理者専用ログイン", type="primary", use_container_width=True):

                admin_id, admin_pass = get_admin_credentials()

                if admin_id_input == admin_id and admin_pass_input == admin_pass:
...
def show_register():
    ...
    with st.container(border=True):
        ...
        password = st.text_input("暗証番号 (数字4桁)", type="password", max_chars=4, placeholder="例: 1234")
        ...
        if st.button("この内容で登録する", type="primary", use_container_width=True):
            ...
            if not (name and phone and password):
                ...
                st.error("電話番号の形式が正しくありません。ハイフンなしの数字で入力してください（例: 09012345678）。")

            elif not is_valid_pin(password):

                st.error("暗証番号は数字4桁で入力してください。")

            elif phone in db["users"]:
...
def show_job_list():
    ...
    city_filter = col_f1.selectbox("市町村で絞り込む", ["すべて"] + akita_cities)

    sort_order = col_f2.selectbox("並び替え", ["新着順", "締切が近い順"])
    ...
    for jid, j in db["jobs"].items():
        ...
        if j.get("status") == "approved" and jid not in user_history:
            ...
            deadline_dt = parse_dt(j.get("deadline_at"))

            expire_dt = parse_dt(j.get("expire_at"))

            if deadline_dt and now >= deadline_dt: continue

            if expire_dt and now >= expire_dt: continue

            if city_filter != "すべて" and city_filter not in j.get("loc", ""): continue

            available_jobs[jid] = j

    if not available_jobs:
st.info("現在募集中の求人はありません（すべて応募済み、条件に合致なし、もしくは期限切れです）。")

    else:

        items = list(available_jobs.items())

        if sort_order == "締切が近い順":

            items.sort(key=lambda kv: parse_dt(kv[1].get("deadline_at")) or datetime.datetime.max)
        ...
        for jid, job in items:
            ...
            with st.container(border=True):
                ...
                deadline_dt = parse_dt(job.get("deadline_at"))
...
def show_job_detail():
    ...
    with st.container(border=True):
        ...
        app_address = col_n2.text_input("詳しい現住所")
        ...
        agree = st.checkbox("応募情報が募集者へ共有されることに同意します（必須）")
        ...
        if st.button("✨ この内容で応募する", type="primary", use_container_width=True):
            ...
            is_too_late = bool(deadline_dt and now >= deadline_dt)
            ...
            if is_too_late:
                ...
                st.warning("個人情報の共有についての同意が必要です。")
...
def show_post_job():
    ...
    with st.container(border=True):
        ...
        job_date = st.date_input("稼働日", value=japan_today, min_value=japan_today)
        ...
        deadline_date = col3.date_input("締め切り日", value=japan_today, min_value=japan_today)
        ...
        if col_z2.button("🔍 住所を検索", use_container_width=True):
            ...
            elif not re.match(r"^\d{7}$", zip_code):

                st.error("郵便番号は数字7桁で入力してください。")
        ...
        if st.button("事務局へ確認申請を提出する", type="primary", use_container_width=True):
            ...
            work_start_dt = datetime.datetime.combine(job_date, start_time)

            work_end_dt = datetime.datetime.combine(job_date, end_time)

            deadline_dt = datetime.datetime.combine(deadline_date, deadline_time)

            now = get_japan_now()

            if not (title and loc_detail):

                st.warning("未入力の項目があります。")

            elif work_end_dt <= work_start_dt:

                st.error("終了時刻は開始時刻より後に設定してください。")

            elif deadline_dt < now:

                st.error("締め切り日時は現在より後に設定してください。")

            elif deadline_dt > work_start_dt:

                st.error("締め切り日時は稼働開始時刻より前に設定してください。")
                ...
                db["jobs"][new_jid] = {
                    ...
                    "expire_at": work_end_dt.strftime("%Y-%m-%d %H:%M:%S"),
...
def show_admin_dashboard():
    ...
    for jid, job in pending_jobs.items():
        ...
        with st.container(border=True):
            ...
            if col2.button("🗑 却下", key=f"del_{jid}", use_container_width=True):
                ...
                st.session_state.confirm_delete = ("job", jid)
    ...
    if st.session_state.confirm_delete and st.session_state.confirm_delete[0] == "job":

        _, target_jid = st.session_state.confirm_delete

        target_job = db["jobs"].get(target_jid)

        if target_job:

            st.warning(f"本当に「{target_job['title']}」を削除しますか？この操作は取り消せません。")

            c1, c2 = st.columns(2)

            if c1.button("✅ はい、削除する", type="primary", use_container_width=True):

                del db["jobs"][target_jid]
    ...
    for phone, u in db["users"].items():
        ...
        with st.container(border=True):
            ...
            if st.button("🗑️ 削除", key=f"del_user_{phone}"):
                ...
                st.session_state.confirm_delete = ("user", phone)

                st.rerun()

    if st.session_state.confirm_delete and st.session_state.confirm_delete[0] == "user":

        _, target_phone = st.session_state.confirm_delete

        target_user = db["users"].get(target_phone)

        if target_user:

            st.warning(f"本当に「{target_user.get('name')}」さんを削除しますか？この操作は取り消せません。")

            c1, c2 = st.columns(2)

            if c1.button("✅ はい、削除する", type="primary", use_container_width=True):

                del db["users"][target_phone]
